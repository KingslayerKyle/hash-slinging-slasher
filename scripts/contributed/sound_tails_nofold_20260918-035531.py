...
the committed tails and heads passes build each Black Ops 4 sound file's variants in a spelling
that cannot hash to its id. **Contributed passes did not have that hole**: `blkops04_sndtails`
(2026-08-23) and `bo4_sound_asset_base_tail3` (2026-08-26 to 09-04) both cut unfolded sound names
short and re-tailed them. This is the same question re-asked on the corpus as it stands now,
seven times larger than when those ran -- see the measurement below for why it was worth one run
and is not worth another.
...
## Measured 2026-09-14: 0 -- record it as a dead end

    known Black Ops 4 sound file names, unfolded and proven: 60,584
    11 distinct encoding tails; alphabet of 36
    k=3: 18,067 stems x 513,216 endings, 1.16 B scanned per pool pass, 0 matched

Not 0 new -- 0 hits of any kind, against the 13,701 Black Ops 4 `sound_asset` ids still
unnamed after claims. Two things worth keeping beyond the zero:

  - **The pool is 82.7% named, not 10.9%.** `coverage.py` subtracts only the tables, and merged
    submissions have named ~52,000 of these files the tables have not absorbed. 60,584 proven
    names is seven times the 8,385 METHODS still argues from.
  - This is the fifth shape to return a total zero on this pool (numbered takes, directory x
    basename, all-boundary cores, alias cells, now tails). Varying the end of a known basename
    does not reach the unnamed files either; they come from outside the naming or not at all.
"""
import argparse
import collections
import itertools
