...
def held_ngrams(max_tokens=MAX_TOKENS):
    """Every multi-token run any held name contains.

    The token test asks whether a *word* is new. This asks whether a *phrase* is -- which is a
    much larger and, measured, a more productive question: the same corpus gives 4,008 cores
    carrying an unseen token and 26,960 carrying an unseen combination of seen ones.
    """
    out = set()
    split = re.compile(r"[^a-z0-9]+")
    for base in ("all_names", os.path.join("cod-name-db", "csv")):
        ...
        for root, _, files in os.walk(d):
            for f in files:
                ...
                with open(os.path.join(root, f), encoding="utf-8", errors="replace") as fh:
                    for line in fh:
                        t = [x for x in split.split(line.strip().split(",", 1)[-1].lower()) if x]
                        for i in range(len(t)):
                            for j in range(i + 2, min(i + max_tokens, len(t)) + 1):
                                out.add("_".join(t[i:j]))
    return out
...
def main():
    ...
    if "--combinations" in sys.argv:
        # The wider question, and the one worth asking second: a phrase of ordinary words that
        # no held name puts together. Six times the stems, from the same corpus.
        multi = {c for c in runs if "_" in c}
        held = held_ngrams()
        print("multi-token runs held: %d" % len(held), file=sys.stderr)
        novel = multi - held
        print("novel (a combination no held name contains): %d" % len(novel), file=sys.stderr)
    else:
        # A core earns its place only if some token in it is one nothing we hold has ever used.
        # A core of entirely familiar tokens is reachable already and would re-grind the corpus.
        held = held_tokens()
