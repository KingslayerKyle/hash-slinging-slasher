"""Rotate every slash-separated component of known non-sound names left by one."""
import os, sys
...
for name in known:
    ...
    if candidate not in known:
        seen.add(candidate)
print(f"{len(known)} known names; {len(seen)} rotated-full-path candidates", file=sys.stderr)
